...
namespace Tests\Portal\CompanySubscription;
...
{
    public function testAdminCanAccessCompanySubscriptions(): void
    {
        ...
        $this->actingAs($this->admin)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ->component('CompanySubscription/Overview/index')
    ...
    public function testCustomerCanNotAccessCompanySubscriptions(): void
    {
    ...
    public function testCanFilterCompanySubscriptions(): void
    {
        ...
        $this->actingAs($this->admin)
            ->get("/companies/subscriptions?id.eq={$data->id}")
            ->assertInertia(fn (Assert $page) => $page
                ->component('CompanySubscription/Overview/index')
                ->has('subscriptions', 1)
                ->has('subscriptions.0', fn (Assert $page) => $page
    ...
    public function testAdminCanAccessCompanySubscriptionsJson(): void
    {
        $response = $this->actingAs($this->admin)
            ->get('/companies/subscriptions/json');
        $keys = array_keys(
    ...
    public function testCanAccessCompanySubscriptionJson(): void
    {
        $subscription = $this->userCompany->subscriptions()->first();
        $response = $this->actingAs($this->admin)
            ->get("/companies/subscriptions/{$subscription->id}/json");
        $keys = array_keys(
            SubscriptionResponseDTO::from(Subscription::first())->toArray()
        );
    ...
    public function testCanNotAccessCompanySubscriptionJson(): void
    {
    ...
    public function testCanAccessCompanySubscriptionWizard(): void
    {
        ...
        $services = Service::whereNotIn('id', [4])
            ->where('type', MembershipTypeEnum::Company())->get();

        $this->actingAs($this->admin)
            ->get('/companies/subscriptions/wizard')
            ->assertInertia(fn (Assert $page) => $page
                ->component('CompanySubscription/Wizard/index')
    ...
    public function testCanCreateCompanySubscriptionFromWizard(): void
    {
        Event::fake(ScheduleCleaningCreated::class);
        $customer = $this->userCompany->customers->first();
        $this->userCompany->subscriptions()->forceDelete();
        $this->userCompany->fixedPrices()->forceDelete();

        $startAt = now()->addDay()->format('Y-m-d');
        $productIds = [1, 2];
        $data = [
            'userId' => $this->userCompany->id,
            'propertyId' => $this->userCompany->properties->first()->id,
        ...
        $this->actingAs($this->admin)
            ->post('/companies/subscriptions/wizard', $data)
            ->assertStatus(302)
            ->assertRedirect()
        ...
        $subscription = Subscription::where('user_id', $this->userCompany->id)->first();
    ...
    public function testCanCreateCompanySubscriptionWithCustomPriceFromWizard(): void
    {
        Event::fake(ScheduleCleaningCreated::class);
        $customer = $this->userCompany->customers->first();
        $this->userCompany->subscriptions()->forceDelete();
    ...
    public function testCanCreateCompanySubscriptionWithExistingFixedPrice(): void
    {
        Event::fake(ScheduleCleaningCreated::class);
        $customer = $this->userCompany->customers->first();
        $this->userCompany->fixedPrices()->forceDelete();
    ...
    public function testCanUpdateCompanySubscription(): void
    {
        Event::fake(ScheduleCleaningCreated::class);
        $subscription = Subscription::where('user_id', $this->userCompany->id)->first();
        ...
        $this->actingAs($this->admin)
            ->patch("/companies/subscriptions/{$subscription->id}", $data)
            ->assertStatus(302)
            ->assertRedirect()
    ...
    public function testCanPauseCompanySubscription(): void
    {
        $subscription = Subscription::where('is_paused', false)->first();

        $this->actingAs($this->admin)
            ->post("/companies/subscriptions/{$subscription->id}/pause")
            ->assertStatus(302)
            ->assertRedirect()
    ...
    public function testCanContinueCompanySubscription(): void
    {
        Event::fake(ScheduleCleaningCreated::class);
        ...
        $this->actingAs($this->admin)
            ->post("/companies/subscriptions/{$subscription->id}/continue")
            ->assertStatus(302)
            ->assertRedirect()
    ...
    public function testCanDeleteCompanySubscription(): void
    {
        $subscription = Subscription::first();

        $this->actingAs($this->admin)
            ->delete("/companies/subscriptions/{$subscription->id}")
            ->assertStatus(302)
            ->assertRedirect()
    ...
    public function testCanRestoreCompanySubscription(): void
    {
        Event::fake(ScheduleCleaningCreated::class);
        $subscription = Subscription::first();
        ScheduleCleaning::whereNotNull('id')->forceDelete();
        ...
        $this->actingAs($this->admin)
            ->post("/companies/subscriptions/{$subscription->id}/restore")
            ->assertStatus(302)
            ->assertRedirect()
