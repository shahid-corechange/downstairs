...
class ScheduleEmployeeController extends Controller
{
    ...
    public function start(
        ...
        $lockKey = 'schedule-employee:start:'.$schedule->id;
        $lockTtlSeconds = 15; // protects critical section; keep short and safe

        Cache::lock($lockKey, $lockTtlSeconds)->block(5, function () use (
            $request,
            $now,
            &$scheduleEmployee,
            &$sendNotif,
            ...
            // Refresh schedule employee and short-circuit if already in progress
            $scheduleEmployee->refresh();
            if ($scheduleEmployee->status == ScheduleEmployeeStatusEnum::Progress()) {
                return; // exit lock block and proceed to controller return
            }

            // Ensure DB-level row lock within the transaction to serialize updates
    ...
    public function end(
        ...
        $lockKey = 'schedule-employee:end:'.$schedule->id;
        $lockTtlSeconds = 15; // protects critical section; keep short and safe
        ...
        Cache::lock($lockKey, $lockTtlSeconds)->block(5, function () use (
            ...
            // Refresh schedule and short-circuit if already done
            $scheduleEmployee->refresh();
    ...
    public function cancel(
        ...
        $lockKey = 'schedule-employee:cancel:'.$scheduleEmployee->schedule_id;
        $lockTtlSeconds = 15; // protects critical section; keep short and safe
        ...
        Cache::lock($lockKey, $lockTtlSeconds)->block(5, function () use (
            ...
            // Refresh schedule employee and short-circuit if already canceled
            $scheduleEmployee->refresh();
            if ($scheduleEmployee->status == ScheduleEmployeeStatusEnum::Cancel()) {
                return; // exit lock block and proceed to controller return
            }
