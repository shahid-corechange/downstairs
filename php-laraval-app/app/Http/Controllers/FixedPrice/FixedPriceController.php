...
class FixedPriceController extends BaseUserController
{
    use MetaTrait;
    use ResponseTrait;
    ...
    public function index(): Response
    {
        ...
        return Inertia::render('Customer/FixedPrice/index', [
    ...
    /**
     * Display the index all view as json.
     */
    public function allJsonIndex(): JsonResponse
    {
        $queries = $this->getQueries();
        $paginatedData = FixedPrice::applyFilterSortAndPaginate($queries);
    ...
    public function store(CreateFixedPriceRequestDTO $request): RedirectResponse
    {
        ...
        $rows = $request->rows->toCollection()->map(
            fn ($row) => [
                ...
                'has_rut' => in_array($row->type, $rutTypes),
            ]
        );
        ...
        DB::transaction(function () use ($request, $rows, $laundryTypes) {
            ...
            // merge cleaning and laundry invoice
            if ($request->type === FixedPriceTypeEnum::CleaningAndLaundry()) {
                $invoices = Invoice::where('user_id', $request->user_id)
                    ->open(InvoiceTypeEnum::Cleaning())
                    ->get();

                foreach ($invoices as $invoice) {
                    $this->invoiceService->merge($invoice);
                }
            }
    ...
    public function update(
        UpdateFixedPriceRequestDTO $request,
    ...
    public function destroy(FixedPrice $fixedPrice): RedirectResponse
    {
        DB::transaction(function () use ($fixedPrice) {
            ...
            if ($fixedPrice->type === FixedPriceTypeEnum::CleaningAndLaundry()) {
                // split cleaning and laundry invoice if not include laundry
                $invoices = Invoice::where('user_id', $fixedPrice->user_id)
                    ->open(InvoiceTypeEnum::CleaningAndLaundry())
                    ->get();

                foreach ($invoices as $invoice) {
                    $this->invoiceService->split($invoice);
                }
            }
        });
    ...
    public function restore(FixedPrice $fixedPrice): RedirectResponse
    {
        ...
        DB::transaction(function () use ($fixedPrice) {
            ...
            if ($fixedPrice->type === FixedPriceTypeEnum::CleaningAndLaundry()) {
                // merge cleaning and laundry invoice if include laundry
                $invoices = Invoice::where('user_id', $fixedPrice->user_id)
                    ->open(InvoiceTypeEnum::Cleaning())
                    ->get();
