...
const getCompanyCustomerDiscount = (
  options: Partial<RequestQueryStringOptions<CustomerDiscount>>,
) => {
  router.get("/companies/discounts" + createQueryString(options), undefined, {
    preserveState: true,
  });
};
    ...
    <>
      ...
      <MainLayout content={{ p: 6 }}>
        ...
        <DataTable
          ...
          fetchFn={getCompanyCustomerDiscount}
          withCreate={hasPermission("company discounts create")}
          withEdit={(row) =>
            hasPermission("company discounts update") && !row.original.deletedAt
          }
          withDelete={(row) =>
            hasPermission("company discounts delete") && !row.original.deletedAt
          }
          withRestore={(row) =>
            hasPermission("company discounts restore") &&
