...
import { CompanyFixedPriceProps } from "@/pages/Company/FixedPrice/types";
...
const EditForm = ({ fixedPrice, row, onCancel, onRefetch }: EditFormProps) => {
  const { t } = useTranslation();
  const { errors: serverErrors } =
    usePage<PageProps<CompanyFixedPriceProps>>().props;
  const {
    register,
    watch,
    ...
    values: {
      ...
      laundryProductIds:
        row.type === "laundry"
          ? JSON.stringify(
              (fixedPrice.laundryProducts || []).map((item) => item.id),
            )
          : undefined,
    },
  });
  ...
  const rowTypeOptions = useMemo(
    () =>
      FIXED_PRICE_ROW_TYPES.filter((value) => {
        ...
        return type === value || !selectedRowTypes.includes(value);
      }).map((value) => ({
        label: t(value),
        value: value,
      })),
    [selectedRowTypes, type],
  );
  ...
  const handleSubmit = formSubmitHandler((values) => {
    ...
    router.patch(
      `/companies/fixedprices/${fixedPrice.id}/rows/${row.id}`,
      {
        ...values,
        laundryProductIds:
