...
const RowPanel = ({
  ...
  const handleDelete = () => {
    ...
    router.delete(
      `/companies/fixedprices/${fixedPrice.id}/rows/${selectedRow?.id}`,
      {
  ...
  return (
    <TabPanel {...props}>
      ...
      <DataTable
        ...
        filterable={false}
        paginatable={false}
        withCreate={hasPermission("company fixed price rows create")}
        withEdit={hasPermission("company fixed price rows update")}
        withDelete={hasPermission("company fixed price rows delete")}
        onCreate={() =>
          onModalExpansion({
            title: t("add row"),
            content: (
