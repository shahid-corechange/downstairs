...
const EditModal = ({ data, onClose, isOpen }: EditModalProps) => {
  ...
  const handleSubmit = formSubmitHandler(({ rows, ...values }) => {
    ...
    router.put(`/price-adjustments/${data?.id}`, newValues, {
      onFinish: () => {
  ...
  const inputComposerDefaultValues = useMemo(() => {
    return (
      data?.rows?.map((row) => ({
        id: String(row.adjustableId),
  ...
  return (
    <Modal title={t("edit price adjustment")} isOpen={isOpen} onClose={onClose}>
      <Flex
        as="form"
        direction="column"
        gap={4}
      ...
      >
        ...
        <InputComposer
          ...
          defaultValues={inputComposerDefaultValues}
