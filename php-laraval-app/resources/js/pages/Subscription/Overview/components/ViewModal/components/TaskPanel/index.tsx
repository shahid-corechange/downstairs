...
const TaskPanel = ({
  ...
  const handleDelete = () => {
    ...
    router.delete(
      `/customers/subscriptions/${subscription.id}/tasks/${selectedTask?.id}`,
      {
  ...
  return (
    <TabPanel {...props}>
      <DataTable
        ...
        filterable={false}
        paginatable={false}
        withCreate={hasPermission("subscription tasks create")}
        withEdit={hasPermission("subscription tasks update")}
        withDelete={hasPermission("subscription tasks delete")}
        onCreate={() =>
          onModalExpansion({
            content: (
              <AddForm
