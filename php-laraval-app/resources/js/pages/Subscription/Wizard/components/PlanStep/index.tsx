...
import { useGetCustomerFixedPrices } from "@/services/fixedPrice";

import { PageProps } from "@/types";

import {
  CustomerSubscriptionWizardPageProps,
  PlanFormValues,
  StepsValues,
} from "../../types";
...
const PlanStep = () => {
  ...
  const {
    ...
    query,
    errors: serverErrors,
  } = usePage<PageProps<CustomerSubscriptionWizardPageProps>>().props;
  ...
  const fixedPrices = useGetCustomerFixedPrices({
    request: {
      size: -1,
      show: "active",
      include: ["user", "rows"],
  ...
  const teamOptions = useMemo(
    () =>
      teams.map((item) => {
        const length = item.users?.length ?? 0;
    ...
    >
      ...
      <Autocomplete
        ...
        errorText={
          ...
          serverErrors["cleaningDetail.propertyId"]
        }
        value={propertyId}
        {...register("propertyId", {
          required: t("validation field required"),
      ...
      <Flex gap={4}>
        <Input
          ...
          min={1}
          errorText={
            errors.quarters?.message || serverErrors["cleaningDetail.quarters"]
          }
          {...register("quarters", {
            required: t("validation field required"),
