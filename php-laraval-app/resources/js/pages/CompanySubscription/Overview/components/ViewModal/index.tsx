...
import { useGetCompanySubscription } from "@/services/companySubscription";
...
const ViewModal = ({ subscriptionId, isOpen, onClose }: ViewModalProps) => {
  ...
  const subscription = useGetCompanySubscription(subscriptionId, {
    request: {
      include: ["tasks"],
      only: [
        "id",
    ...
    >
      {!subscription.isFetching && subscription.data ? (
        <Tabs index={activeTabIndex} onChange={handleChangeTab}>
          <TabList>
            <AuthorizationGuard permissions="company subscription tasks index">
              <Tab>{t("tasks")}</Tab>
            </AuthorizationGuard>
          </TabList>
          <TabPanels>
