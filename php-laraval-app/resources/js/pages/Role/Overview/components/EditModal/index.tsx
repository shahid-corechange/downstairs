...
const EditModal = ({ data, onClose, isOpen }: EditModalProps) => {
  ...
  const defaultPermissions = useMemo(
    () =>
      data && data.permissions
        ? data.permissions.map((permission) => permission.name as PermissionKey)
        : [],
    [data],
  );
  ...
  useEffect(() => {
    if (isOpen) {
      setTimeout(
        () => reset({ name: data?.name, permissions: defaultPermissions }),
        0,
      );
    }
  ...
  return (
    <Modal title={t("edit role")} isOpen={isOpen} onClose={onClose}>
      ...
      >
        <Input
          labelText={t("name")}
          isDisabled={data?.name === "Employee" || data?.name === "Customer"}
